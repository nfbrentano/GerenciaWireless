                    ...
                    <%@page import="java.sql.ResultSet" %>
                        <%-- Para poder utilizar a jstl é necessário adicionar o respectivo .jar nas bibliotecas --%>
                                        ...
                                        <script>
                                            ...
                                            < !--Função javascript para chamar Servlet de exclusão de registro-- >
                                                function excluirRegistroBairro(idbairro, nome) {
                                                    if (confirm('Confirma exclus?o do bairro de código: ' + idbairro + ' - Nome:
                                            ' + nome + ' ? ')) {
                                            window.location.href = "<%=request.getContextPath()%>/ExcluirBairro?cod=" +
                                                        idbairro;
                                                } else {
                                        ...
                                        <form name="formTeste" class="form-register1">
                                            ...
                                            <%@ include file="nav.jsp" %>
                                                ...
                                                <form id="custom-search-form"
                                                    class="form-search form-horizontal pull-right" name="frmLocalizar"
                                                    action="listarBairros.jsp" method="POST">
                                                    <div class="panel-body">
                                                        <div class="input-group">

                                                            <input class="form-control"
                                                                placeholder="Buscar pelo nome..." type="text"
                                                                name="localizaBairro" value="">
                                                            <div class="input-group-btn">
                                                                <button class="btn btn-default" type="submit"
                                                                    value="Pesquisar" name="btnLocaliza"><span
                         ...
                         Cada linha da tabela apresentar? um registro da tabela "clientes" -->
                                                        <% try { // Conectar o banco usando o utilitário padronizado
                                                            Connection conn=util.Db.getConexao(); try { String
                                                            query="SELECT idbairro, nome, cidade_idcidade FROM bairro WHERE disponibilidade = true "
                                                            ; String
                                                            localizarValor=request.getParameter("localizaBairro"); if
                                                            (localizarValor !=null && !localizarValor.isEmpty()) { query
                                                            +=" AND LOWER(nome) LIKE ?" ; } try
                                                            ...
                                                            // Processar cada item do banco e adicionar uma linha na
                                                            tabela
                                                            while (rs.next()) {
                                                            %>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <%= rs.getString("idbairro")%>
                                                                </td>
                                                                <td>
                                                                    <%= rs.getString("nome")%>
                                                                </td>
                                                                <td>
                                                                    <%= rs.getString("cidade_idcidade")%>
                                                                </td>
                                                                <td class="text-center"> <a class='btn btn-info btn-xs'
                                                                        onclick="editarRegistroBairro(<%= rs.getInt("
                                                                        idbairro")%>);"><span
                                                                            class="glyphicon glyphicon-edit"></span>
                                                                        Editar</a> </td>
                                                                <td class="text-center"> <a
                                                                        class='btn btn-danger btn-xs'
                                                                        onclick="excluirRegistroBairro(<%= rs.getInt("
                                                                        idbairro")%>, '<%= rs.getString("nome")%>
                                                                            ');"><span
                                                                                class="glyphicon glyphicon-remove"></span>
                                                                            Del</a></td>

                                                            </tr>
                                                            <% <% } // Fim do while } } } catch (Exception e) {
                                                                out.write("Ocorreu um erro ao buscar os bairros <span
